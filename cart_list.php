                    ...
                    <div class="card-body">
                        <div class="container-fluid">
                            <div id="item_list" class="list-group">
                                ...
                                <?php 
                                $gt = 0;
                                $cart = $conn->query("SELECT c.*, p.name as product, p.brand as brand, p.revenue as rev, p.price, cc.name as category, p.image_path, (COALESCE((SELECT SUM(quantity) FROM `stock_list` where product_id = p.id and (expiration IS NULL or date(expiration) > '".date("Y-m-d")."') ), 0) - COALESCE((SELECT SUM(quantity) FROM `order_items` where product_id = p.id), 0)) as `available` FROM `cart_list` c inner join product_list p on c.product_id = p.id inner join category_list cc on p.category_id = cc.id where customer_id = '{$_settings->userdata('id')}' ");
                                    ...
                                    <div class="d-flex w-100 align-items-center">
                                        ...
                                        <div class="col-auto flex-shrink-1 flex-grow-1">
                                            <div style="line-heigth:1em">
                                                ...
                                                <div class="text-muted"><?= $row['rev'] ?></div>
