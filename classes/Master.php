...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		if($insert){
			...
			$data = "";
			$cart = $this->conn->query("SELECT c.*, p.name as product, p.brand as brand, p.revenue as revenue, p.price, cc.name as category, p.image_path, COALESCE((SELECT SUM(quantity) FROM `stock_list` where product_id = p.id and (expiration IS NULL or date(expiration) > '".date("Y-m-d")."') ), 0) as `available` FROM `cart_list` c inner join product_list p on c.product_id = p.id inner join category_list cc on p.category_id = cc.id where customer_id = '{$customer_id}' ");
			while($row = $cart->fetch_assoc()):
				if(!empty($data)) $data .= ", ";
				$data .= "('{$oid}', '{$row['product_id']}', '{$row['quantity']}', '{$row['price']}', '{$row['revenue']}')";
			endwhile;
			if(!empty($data)){
				$sql = "INSERT INTO order_items (`order_id`, `product_id`, `quantity`, `price`, `revenue`) VALUES {$data}";
				$save = $this->conn->query($sql);
				if($save){
					$resp['status'] = 'success';
