...
<div class="container-fluid">
    ...
    <div id="item_list" class="list-group">
        <?php 
        $gt = 0;
        $order_items = $conn->query("SELECT o.*, p.name as product, p.brand as brand,  p.revenue as revenue, p.price, cc.name as category, p.image_path, COALESCE((SELECT SUM(quantity) FROM `stock_list` where product_id = p.id and (expiration IS NULL or date(expiration) > '".date("Y-m-d")."') ), 0) as `available` FROM `order_items` o inner join product_list p on o.product_id = p.id inner join category_list cc on p.category_id = cc.id where order_id = '{$id}' ");
        while($row = $order_items->fetch_assoc()):
            $gt += $row['revenue'] * $row['quantity'];
        ?>
                ...
                <div class="col-auto flex-shrink-1 flex-grow-1">
                    <div style="line-heigth:1em">
                        ...
                        <div class="text-muted d-flex w-100">
                            <?= format_num($row['quantity'],0) ?> x <?= format_num($row['revenue'],2) ?>
                        </div>
                    </div>
                </div>
                <div class="col-auto">
                    <h4><b>RM <?= format_num($row['revenue'] * $row['quantity'], 2) ?></b></h4>
                </div>
            </div>
        </div>
        <?php endwhile; ?>
    ...
    <div class="d-flex justify-content-end py-3">
        <div class="col-auto">
            <h3><b>Total Profit: RM <?= format_num($gt,2) ?></b></h3>
        </div>
    </div>
</div>
<hr class="px-n5">
